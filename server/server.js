...
const { migrateChatTables } = require('./migrations/chatMigration');
...
app.post('/api/chat/message', async (req, res) => {
  try {
    const { message, chatMode, supportTicketId, userId } = req.body;
    ...
    // Save user message
    await runSql(`
      INSERT INTO chat_messages (user_id, support_ticket_id, message, sender_type, timestamp)
    ...
    if (chatMode === 'bot') {
      ...
      botResponse = await generateBotResponse(message, userId);
      ...
      `, [userId, ticketId, botResponse.response]);
      
    } else {
      // Human support mode
      response = {
        response: 'Thank you for your message. A support agent will respond shortly. Average response time is 5-10 minutes during business hours.',
        suggestions: []
      };
      
      // Update ticket to indicate human support needed
      await runSql(`
        UPDATE support_tickets 
        SET needs_human_support = 1, updated_at = datetime('now')
        WHERE id = ?
      `, [ticketId]);
    }
    ...
    res.json({
      ...
      switchToHuman: botResponse.switchToHuman || false
    });
...
// Get chat history
app.get('/api/chat/history/:ticketId', async (req, res) => {
  try {
    const { ticketId } = req.params;
    
    const messages = await allSql(`
      SELECT message, sender_type, timestamp, admin_name
      FROM chat_messages 
...
app.get('/api/admin/support/tickets', async (req, res) => {
  try {
    ...
    let query = `
      ...
      LEFT JOIN chat_messages cm ON st.id = cm.support_ticket_id
    `;
    ...
    query += `
      GROUP BY st.id, st.user_id, st.status, st.needs_human_support, st.created_at, st.updated_at, u.name, u.email
    ...
    const tickets = await allSql(query, params);
...
app.post('/api/admin/support/respond', async (req, res) => {
  try {
    const { ticketId, message } = req.body;
    const adminName = 'Support Agent'; // In real app, get from authenticated user
    ...
    `, [ticketId, message.trim(), adminName]);
...
// Bot response generator function with Python guard integration
async function generateBotResponse(message, userId) {
  const lowerMessage = message.toLowerCase();
  
  // Try to get response from Python guard service first
  try {
    const guardResponse = await fetch('http://localhost:8001/chat', {
      method: 'POST',
      headers: {
    ...
    if (guardResponse.ok) {
      ...
      if (guardData.response && guardData.response.trim()) {
        return {
          response: guardData.response,
          suggestions: guardData.suggestions || [],
          switchToHuman: guardData.switchToHuman || false,
          source: 'python_guard'
        };
      }
    }
  } catch (error) {
    console.log('Python guard service not available, using fallback logic:', error.message);
  }
  
  // Fallback to local FAQ responses if Python guard is not available
  const faqResponses = {
    'track my order': {
      response: 'I can help you track your order! You can view all your orders and their current status in the "You" section under Order History. Each order shows real-time tracking information.',
      suggestions: ['View order history', 'Contact seller', 'Shipping information']
    },
    'return': {
      response: 'For returns and exchanges:\\n• Items can be returned within 7 days of delivery\\n• Items must be in original condition\\n• Contact the seller first through their profile\\n• If unresolved, our team can help mediate',
      suggestions: ['Return policy', 'Contact seller', 'Dispute resolution', 'Talk to human support']
    },
    'payment': {
      response: 'Having payment issues? Here are common solutions:\\n• Check if your payment method is valid\\n• Ensure sufficient funds/credit limit\\n• Try a different payment method\\n• Clear browser cache and try again',
      suggestions: ['Payment methods', 'Transaction failed', 'Refund status', 'Talk to human support']
    },
    'seller': {
      response: 'Seller questions I can help with:\\n• How to list products\\n• Managing inventory\\n• Seller dashboard features\\n• Commission structure\\n• Shipping guidelines',
      suggestions: ['How to sell', 'Product listing', 'Seller fees', 'Shipping help']
    },
    'account': {
      response: 'Account help available:\\n• Profile management\\n• Password reset\\n• Email verification\\n• Account security\\n• Privacy settings',
      suggestions: ['Reset password', 'Update profile', 'Security settings', 'Delete account']
    }
  };
  
  // Check for greetings
  if (['hi', 'hello', 'hey', 'good morning', 'good afternoon'].some(greeting => lowerMessage.includes(greeting))) {
    return {
      response: 'Hello! How can I assist you with your ThriftIt experience today?',
      suggestions: ['Track my order', 'Return/Exchange', 'Payment issues', 'Seller questions', 'Account help']
    };
  }
  
  // Check for human support request
  if (['human', 'agent', 'person', 'representative', 'talk to someone'].some(human => lowerMessage.includes(human))) {
    return {
      response: 'I\'ll connect you with a human support agent. Please wait a moment...',
      suggestions: [],
      switchToHuman: true
    };
  }
  ...
  // Default response
  return {
    response: 'I understand you need help with that. Let me connect you with a human support agent who can better assist you with your specific question.',
    suggestions: ['Talk to human support', 'Browse FAQ', 'Try different keywords'],
    switchToHuman: true
  };
}

// Test endpoint for Python guard integration
app.get('/api/chat/test-guard', async (req, res) => {
  try {
    ...
    if (guardResponse.ok) {
      ...
      res.json({
        ...
        error: `Python guard service responded with status: ${guardResponse.status}`,
        testMessage: testMessage
      });
    }
  } catch (error) {
    res.json({
      ...
      guardAvailable: false,
      error: error.message,
      testMessage: req.query.message || 'Hello, this is a test message'
    });
  }
});
...
const startServer = async () => {
  try {
    ...
    console.log("✅ Chat and support system initialized");
